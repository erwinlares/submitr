...
in the `toolero` repository. Two of them appear throughout this
vignette: a derived analysis script lives at `R/analysis.R`, and
analysis outputs go in `output/`. Following them is what lets one script
run unchanged on your laptop and on an execute node.
...
read it to work out which files to move. There is a section on that
below.
...
session so all subsequent calls, whether uploads, submits, status checks
or downloads, reuse the same connection without prompting again. The
setup takes two minutes and is worth doing before your first
...
Note the relative path. `"output/"` resolves correctly in RStudio, under
`quarto render`, and on an execute node, because the generated
executable script moves to HTCondor’s writable scratch directory before
calling `Rscript`. The script itself never has to know where it is
running.
...
information, and it displays ControlMaster setup instructions. It then
stores the configuration for the rest of the session, which is why none
of the calls that follow need a `config` argument.
...
That middle line comes from a short SSH connection made to tell you
...
first established; everything that follows, including file uploads, job
submission, status checks and result downloads, happens without
prompting for Duo MFA again. Full documentation is at
...
uses it only to work out what to call the results tarball, so that the
name matches the one
[`htc_gen_executable()`](https://erwinlares.github.io/submitr/reference/htc_gen_executable.md)
will tell the job to build. The function never opens the executable
script and never reads the Dockerfile, so the name of your analysis
script is genuinely not something it can infer. Supply `output_files`
yourself if you want a different name.
...
`analysis-results.tar.gz` rather than something containing a `R/` that
does not exist on the execute node.
...
log files, `6302860-0-job.log`, `.err` and `.out`. It knows the
tarball’s name because
[`htc_gen_submit()`](https://erwinlares.github.io/submitr/reference/htc_gen_submit.md)
recorded it, knows the cluster ID because
[`htc_submit()`](https://erwinlares.github.io/submitr/reference/htc_submit.md)
recorded it, and knows which remote directory to look in for the same
reason.
...
it the report arrives as a single self-contained file.
...
The reason it is a file rather than something held in the R session is
...
different input. The companion vignette, *Single Jobs vs Multiple Jobs
on HTCondor*, works through both modes side by side.
...
will not automatically have the same setup. A container image solves
...
high-throughput research computing.

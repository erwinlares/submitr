...
analysis – a summary and visualization of the Palmer Penguins dataset.
In single mode, the analysis runs once over the full dataset. In
multiple mode, it runs once per species, producing independent results
...
`quarto render`, and on HTCondor – because the executable script sets
the working directory to HTCondor’s writable scratch space before
calling `Rscript`.
...
`toolero::write_by_group(manifest = TRUE)` and lists the subset data
...
reads this file via the `queue_from` argument to produce the
`subdatasets.csv` that HTCondor uses to dispatch one job per subset. The
data manifest is only relevant in multiple mode.

The **job manifest** is a session-level R option that accumulates
metadata as you work through the submitr pipeline. It is not a file. It
lives in memory and is cleared when you call
[`htc_start()`](https://erwinlares.github.io/submitr/reference/htc_start.md)
or restart R. Each function in the workflow contributes a piece:
[`htc_gen_submit()`](https://erwinlares.github.io/submitr/reference/htc_gen_submit.md)
records the mode, the output file pattern, and the subset names.
[`htc_gen_executable()`](https://erwinlares.github.io/submitr/reference/htc_gen_executable.md)
records the script name and results folder.
[`htc_submit()`](https://erwinlares.github.io/submitr/reference/htc_submit.md)
records the cluster ID assigned by HTCondor. At the end of the workflow,
[`htc_download()`](https://erwinlares.github.io/submitr/reference/htc_download.md)
reads the job manifest to determine which files to retrieve – no glob
patterns or manual file lists required.
...
| Used by | `htc_gen_submit(queue_from = ...)` | [`htc_download()`](https://erwinlares.github.io/submitr/reference/htc_download.md) |
...
writes results to `results/`, the executable script tars them up, and
...
The script changes to the scratch directory (writable), creates the
results folder, runs the R script using absolute paths to the baked-in
files, and tars the results. The tarball lands in the scratch directory
where HTCondor expects to find it.
...
[`htc_download()`](https://erwinlares.github.io/submitr/reference/htc_download.md)
takes no arguments here because the job manifest has everything it
needs.
[`htc_gen_submit()`](https://erwinlares.github.io/submitr/reference/htc_gen_submit.md)
recorded that this is a single-mode job with `analysis-results.tar.gz`
as the output.
[`htc_submit()`](https://erwinlares.github.io/submitr/reference/htc_submit.md)
recorded the cluster ID. From those two pieces,
[`htc_download()`](https://erwinlares.github.io/submitr/reference/htc_download.md)
...
but not the data – each subset file is transferred at runtime by
HTCondor.
...
`gentoo.csv`) and a data manifest (`manifest.csv`) listing them. This
data manifest is a file on disk – not to be confused with the job
manifest that `submitr` builds in memory during the submission workflow.
...
`${1}` is the subset filename passed by HTCondor – e.g. `adelie.csv`.
The R script receives it as `commandArgs(trailingOnly = TRUE)[1]`. The
tarball is named per-subset so jobs don’t overwrite each other.
...
This is where the job manifest pays off. With three species and three
log file types per job, there are 12 files to retrieve.
[`htc_download()`](https://erwinlares.github.io/submitr/reference/htc_download.md)
constructs the full list automatically: it reads the subset names from
the job manifest, expands the output pattern into
`adelie.csv-results.tar.gz`, `chinstrap.csv-results.tar.gz`, and
`gentoo.csv-results.tar.gz`, and generates the nine log file names from
...
[`htc_download()`](https://erwinlares.github.io/submitr/reference/htc_download.md)
can reconstruct the tarball names without re-reading any files from
disk.

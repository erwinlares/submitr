# submitr: Scaffold and Submit Computational Jobs to CHTC Schedulers

Provides scaffolding tools to help researchers prepare and submit
computational jobs to high-throughput computing (HTC) schedulers.
Generates the files required to run containerized R analyses on
'HTCondor', including submit files and executable scripts, and wraps the
system commands needed to stage files, submit jobs, monitor status, and
retrieve results from a CHTC submit node. A job manifest stored
alongside the project configuration tracks metadata across the workflow,
so that input files can be uploaded and result files downloaded
automatically, without constructing file lists by hand and without
losing the job when the R session ends. Provides 'htc_start()' for
session management and 'htc_config()' for connection details and SSH
connection reuse guidance. Works naturally alongside 'containr' for
container image management and 'toolero' for dataset splitting and
project scaffolding.

## See also

Useful links:

- <https://erwinlares.github.io/submitr/>

- Report bugs at <https://github.com/erwinlares/submitr/issues>

## Author

**Maintainer**: Erwin Lares <erwin.lares@wisc.edu>
([ORCID](https://orcid.org/0000-0002-3284-828X))

Authors:

- Erwin Lares <erwin.lares@wisc.edu>
  ([ORCID](https://orcid.org/0000-0002-3284-828X))
